...
#                     print("questions:- ",df['question'][int(idx)],"(Score: {:.4f}%)".format(score*100))
    ...
    def dataframe_filter_return(self, df, industry=None, country=None, section_data=None):
        ...
        if (df['industry'].isin(industry).any()) and (df['country'].isin(country).any()) and (df['section_data'].isin(section_data).any()):
            ...
            print("Filter Applied = {country:", str(country), ", industry:"+str(industry)+", section_data:"+str(section_data)+"}")
            df = df[(df['industry'].isin(industry)) & (df['country'].isin(country)) & (df['section_data'].isin(section_data))]
            ...
            df = df[(df['country'].isin(country)) & (df['section_data'].isin(section_data))]

        elif (df['industry'].isin(industry).any()) and (df['section_data'].isin(section_data).any()):
            print("AND Filter Applied = {country:", str(country), ", industry:"+str(industry)+"}")
            df = df[(df['industry'].isin(country)) & (df['section_data'].isin(section_data))]
