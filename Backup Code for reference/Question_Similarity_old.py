...
class preprocessing:
    ...
    def normalize_text_data(self, text_data):
        ...
        text_data=x.strip()                       # Removing leftmost and rightmost spaces
...
#         text = prep.remove_punctuation(question)
        ...
        dict_words = {'HCM':'Human Capital Management', 'ERP': 'Enterprise Resource Planning', 'SCM': 'Supply Chain Management'}
    ...
    def dataset_preprocessing_and_embedding(self, df, embedder):
        ...
        df['Preprocessed_Question'] = df["Question"].apply(self.preprocess_example)
        
        
        corpus_embeddings = embedder.encode(list(df['Preprocessed_Question']), convert_to_tensor=True)

        # Checking the Directories structures 
        if os.path.isdir('RFP/Embedding Models/'):
            print("-------------------RFP/Embedding Models/ is present------------------------")
        else:
            print("--------------------------Directory is not present-----------")
            os.mkdir('RFP/Embedding Models/')

        if os.path.isdir('RFP/Preprocessed_Files/'):
            print("-------------------RFP/Preprocessed_Files/ is present------------------------")
        else:
            print("--------------------------Directory is not present-----------")
            os.mkdir('RFP/Preprocessed_Files/')
        
        torch.save(corpus_embeddings, "RFP/Embedding Models/corpus_embedding"+'-'+self.model_name+'-'+self.country+"-"+self.industry+".pt")
        df.to_excel('./RFP/Preprocessed_Files/Preprocessed-'+self.model_name+'-'+self.country+'-'+self.industry+".xlsx", index=False)
    ...
    def new_query_preprocess_similarity(self, df, query, embedder,  top=3, threshold=0.5):
        ...
        if os.path.isdir('RFP/Embedding Models/'):
            print("-------------------RFP/Embedding Models/ is present------------------------")
            corpus_embeddings = torch.load('RFP/Embedding Models/corpus_embedding'+'-'+self.model_name+'-'+self.country+'-'+self.industry+'.pt')   
        
            # Find the closest 5 sentences of the corpus for each query sentence based on cosine similarity
            top_k = min(top, len(corpus_embeddings))
                
            query_embedding = embedder.encode(query_preprocessed, convert_to_tensor=True)
...
#                     print("Questions:- ",df['Question'][int(idx)],"(Score: {:.4f}%)".format(score*100))
                    print("Document Link:- ",df['Document_Link'][int(idx)],"(Score: {:.4f}%)".format(score*100))
    ...
    def dataframe_filter_return(self, df, industry=None, country=None, section=None):
        # Filtering Data based on industry, Country and Section
        if (df['Industry'].isin(industry).any()) and (df['Country'].isin(country).any()) and (df['Section'].isin(section).any()):
            print("All three filter applied (Industry, Country, Section)")
            print("Filter Applied = {Country:", str(country), ", Industry:"+str(industry)+", Section:"+str(section)+"}")
            df = df[(df['Industry'].isin(industry)) & (df['Country'].isin(country)) & (df['Section'].isin(section))]

        elif (df['Industry'].isin(industry).any()) and (df['Country'].isin(country).any()):
            print("All three filter applied (Industry, Country)")
            print("Filter Applied = {Country:", str(country), ", Industry:"+str(industry)+"}")
            df = df[(df['Industry'].isin(industry)) & (df['Country'].isin(country))]

        elif (df['Country'].isin(country).any()) and (df['Section'].isin(section).any()):
            print("AND Filter Applied = {Country:", str(country), ", Industry:"+str(industry)+"}")
            df = df[(df['Country'].isin(country)) & (df['Section'].isin(section))]

        elif (df['Industry'].isin(industry).any()) and (df['Section'].isin(section).any()):
            print("AND Filter Applied = {Country:", str(country), ", Industry:"+str(industry)+"}")
            df = df[(df['Industry'].isin(country)) & (df['Section'].isin(section))]

        elif (df['Industry'].isin(industry).any()) or (df['Country'].isin(country).any()) or (df['Section'].isin(section).any()):
            print("Any one of three filter applied (Industry, Country, Section)")
            print("Filter Applied = {Country:", str(country), ", Industry:"+str(industry)+", Section:"+str(section)+"}")
            df = df[(df['Industry'].isin(industry)) | (df['Country'].isin(country)) | (df['Section'].isin(section))]
        else:
    ...
    def query_resolved(self, query, df, embedder):
        start = time.time()
        index_list, cos_sim_list = self.new_query_preprocess_similarity(df, query, top=3, embedder=embedder)
        end = time.time()
